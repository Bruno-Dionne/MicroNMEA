...
public:
	...
	static unsigned long parseUnsignedLong(const char *s, uint8_t len);
	...
	/**
	 * @brief Inquire if latest fix is valid based on RMC message "or" GGA message
	 *
	 * @return true Valid
	...
	/**
	 * @brief Inquire if latest Data valid based on message RMC only
	 *
	 * @return true Valid
	 * @return false Not valid
	 */
	...
	/**
	 * @brief Inquire if latest Position Fix is valid based on message GGA only
	 *
	 * @return true Valid
	 * @return false Not valid
	 */
	bool isFixValid(void) const {
		return _isFixValid;
	}
	...
	/**
	 * @brief Get the Millis part of the second
	 *
	 * @return uint16_t getMillis
	 */
	uint16_t getMillis(void) const {
		return _millis;
	}

	/**
	 * @brief Set the Millis part of the second
	 *
	 * @return none
	 */
	void setMillis( uint16_t newMillis) {
		_millis = newMillis;
	}
	 ...
	 * @return none
	 */
	void setMicros( uint32_t newMicros) {
		_micros = newMicros;
	}
